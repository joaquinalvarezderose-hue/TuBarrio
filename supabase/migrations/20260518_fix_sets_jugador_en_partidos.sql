-- ============================================================
-- Fix: sets_jugador1 / sets_jugador2 nunca se escribían en partidos
--
-- Causa: validar_resultado_seguro actualizaba solo estado/resultado/ganador_id.
-- Los campos sets_jugador1 y sets_jugador2 quedaban en 0 (DEFAULT),
-- por lo que el historial de torneos siempre mostraba "0/0 ganados".
--
-- Este migration:
--   1. Backfill de registros existentes desde torneo_partidos_historial.
--   2. Redeploy de validar_resultado_seguro con el campo corregido.
-- ============================================================

-- ─── 1. Backfill ────────────────────────────────────────────────────────────
UPDATE public.partidos p
SET sets_jugador1 = h.sets_jugador1,
    sets_jugador2 = h.sets_jugador2
FROM public.torneo_partidos_historial h
WHERE h.partido_id = p.id
  AND p.estado = 'finalizado'
  AND p.sets_jugador1 = 0
  AND p.sets_jugador2 = 0;
...
BEGIN
  ...
  -- FIX: ahora también guarda sets_jugador1/sets_jugador2 en partidos
  UPDATE public.partidos

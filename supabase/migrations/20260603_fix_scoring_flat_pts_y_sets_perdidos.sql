-- ============================================================
-- Fix: puntuación plana por partido + sets_perdidos no escrito
--
-- Problema 1: validar_resultado_seguro usaba puntuación diferencial
--   por sets (2-0 → 3pts, 2-1 → 2pts, 1-2 → 1pt, 0-2 → 0pts).
--   La regla correcta es plana: ganar = 3pts, perder = 1pt.
--
-- Problema 2: el UPDATE a torneo_jugadores nunca escribía
--   sets_perdidos, dejándolo en 0 para todos los jugadores y
--   rompiendo los tiebreakers por diferencia de sets.
--
-- Este migration:
--   1. Backfill de torneo_jugadores con puntos y sets_perdidos
--      correctos, recalculados desde torneo_partidos_historial.
--   2. Backfill de torneo_partidos_historial corrigiendo los campos
--      puntos_jugador1 / puntos_jugador2 ya grabados.
--   3. Redeploy de validar_resultado_seguro con la fórmula corregida.
-- ============================================================


-- ─── 1. Backfill torneo_jugadores ───────────────────────────────────────────
UPDATE public.torneo_jugadores tj
SET
  puntos           = sub.total_puntos,
  sets_perdidos    = sub.total_sets_perdidos
FROM (
  SELECT
    tj2.perfil_id,
    h.torneo_id,
    ...
    SUM(
      CASE
        WHEN h.jugador1_perfil_id = tj2.perfil_id
          THEN CASE WHEN h.ganador_perfil_id = h.jugador1_perfil_id THEN 3 ELSE 1 END
        ELSE
          CASE WHEN h.ganador_perfil_id = h.jugador2_perfil_id THEN 3 ELSE 1 END
      END
    )::integer AS total_puntos,
    SUM(
      CASE
        WHEN h.jugador1_perfil_id = tj2.perfil_id THEN h.sets_jugador2
        ELSE h.sets_jugador1
      END
    )::integer AS total_sets_perdidos
  FROM public.torneo_partidos_historial h
  JOIN public.torneo_jugadores tj2
    ON (tj2.perfil_id = h.jugador1_perfil_id OR tj2.perfil_id = h.jugador2_perfil_id)
   AND tj2.torneo_id = h.torneo_id
   AND tj2.categoria = h.categoria
   AND tj2.grupo     = h.grupo
  GROUP BY tj2.perfil_id, h.torneo_id, h.categoria, h.grupo
) sub
WHERE tj.perfil_id  = sub.perfil_id
  AND tj.torneo_id  = sub.torneo_id
  AND tj.categoria  = sub.categoria
  AND tj.grupo      = sub.grupo;


-- ─── 2. Backfill torneo_partidos_historial ──────────────────────────────────
UPDATE public.torneo_partidos_historial h
SET
  puntos_jugador1 = CASE WHEN h.ganador_perfil_id = h.jugador1_perfil_id THEN 3 ELSE 1 END,
  puntos_jugador2 = CASE WHEN h.ganador_perfil_id = h.jugador2_perfil_id THEN 3 ELSE 1 END
WHERE h.ganador_perfil_id IS NOT NULL;

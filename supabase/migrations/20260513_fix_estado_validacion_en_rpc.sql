-- ============================================================
-- Fix: obtener_estado_jugador_torneo now recognises matches in
-- 'esperando_validacion' so it returns 'activo' (not
-- 'esperando_siguiente_ronda') while a result is awaiting the
-- rival's confirmation.
-- ============================================================
...
BEGIN
  ...
  --    (include esperando_validacion so a player whose first
  --    match is pending confirmation is not mis-classified)
  -- --------------------------------------------------------
  IF v_total = 0 THEN
    SELECT EXISTS (
      ...
      WHERE torneo_id = p_torneo_id
        AND (jugador1_id = p_perfil_id OR jugador2_id = p_perfil_id)
        AND estado IN ('programado', 'esperando_validacion')
    ) INTO v_has_pending;
  ...
  --    Includes 'esperando_validacion' so a submitted-but-
  --    unconfirmed result is returned as the current match
  --    (with its real estado) rather than being ignored.
  --    'esperando_validacion' is prioritised so it surfaces
  --    before any freshly-scheduled next-round match.
  -- --------------------------------------------------------
  SELECT
    p.id,
    p.jornada,
  ...
  WHERE p.torneo_id = p_torneo_id
    ...
    AND p.estado IN ('programado', 'esperando_validacion')
  ORDER BY
    -- Surface pending-confirmation matches first so they are
    -- not hidden by a freshly-scheduled next-round match.
    CASE p.estado WHEN 'esperando_validacion' THEN 0 ELSE 1 END,

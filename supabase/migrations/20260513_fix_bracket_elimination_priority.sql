-- ============================================================
-- Fix: bracket elimination check must run BEFORE pending-match
-- search to prevent eliminated players from being returned as
-- 'activo' when phantom future bracket slots are pre-generated.
-- ============================================================
...
BEGIN
  ...
  --     pending matches. This prevents phantom pre-generated
  --     future-round slots from masking elimination status.
  -- --------------------------------------------------------
  SELECT
  ...
  -- --------------------------------------------------------
  -- 4. No pending match (or already bracket-eliminated) →
  --    compute full bracket context for stage name
  -- --------------------------------------------------------
  -- v_bracket_played / v_bracket_wins / v_bracket_losses / v_max_ronda
  -- were already computed in step 2.5 — no need to re-query.
  ...
  --    champion or waiting for next round
  -- --------------------------------------------------------
  ...
  -- --------------------------------------------------------
  -- 7. Only group-stage matches, no bracket, no next match →
  --    waiting for playoffs to be generated
  -- --------------------------------------------------------

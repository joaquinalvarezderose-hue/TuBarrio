...
begin
  ...
  -- Enforce jornada ordering for round-robin matches only (self check)
  ...
  if coalesce(v_partido.bracket_tipo, '') != 'eliminacion_directa' then
    if p_user_id = v_partido.jugador1_id then
      ...
      v_rival_id := v_partido.jugador1_id;
    end if;

    if v_rival_id is not null then
      select min(p.jornada)
      into v_rival_min_jornada
      from public.partidos p
      where p.torneo_id = v_partido.torneo_id
        and p.categoria = v_partido.categoria
        and (p.jugador1_id = v_rival_id or p.jugador2_id = v_rival_id)
        and coalesce(p.bracket_tipo, '') != 'eliminacion_directa'
        and p.estado not in ('finalizado', 'esperando_validacion');

      if v_rival_min_jornada is not null and v_partido.jornada > v_rival_min_jornada then
        return 'Tu rival todavia no cerro la jornada ' || v_rival_min_jornada ||
               '. Espera que complete su partido pendiente antes de cargar este resultado.';
      end if;
    end if;
  end if;

-- Remove jornada-ordering enforcement from enviar_resultado_seguro.
-- Players often play jornadas out of order (vacations, scheduling convenience),
-- so both the self-check and the rival-check that blocked submitting a later
-- jornada before an earlier one was finalized are dropped. All other validations
-- (match exists, participant check, not-already-finalizado) are unchanged.

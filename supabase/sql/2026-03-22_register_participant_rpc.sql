...
begin
  ...
  if not v_ya_inscripto then
    insert into public.torneo_jugadores (perfil_id, categoria, grupo, puntos, partidos_jugados, sets_ganados)
    values (p_perfil_id, p_categoria, p_grupo, 0, 0, 0);
  end if;
  ...
  set current_participantes = (
        ...
        from public.torneo_jugadores tj
        where tj.categoria = p_categoria
          and tj.grupo = p_grupo
      ),
      updated_at = now()
  where te.torneo_id = p_torneo_id
  ...
  select exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'jugador1_perfil_id'
    ...
    while v_i <= array_length(v_shuffled, 1) loop
      if v_i = array_length(v_shuffled, 1) then
        ...
        if v_can_insert_partidos then
          begin
            insert into public.partidos (torneo_id, categoria, grupo, jugador1_perfil_id, jugador2_perfil_id, ronda, estado)
            values (p_torneo_id, p_categoria, p_grupo, v_shuffled[v_i], v_shuffled[v_i + 1], 1, 'PROGRAMADO');
            v_created := v_created + 1;
          exception when others then
            raise notice 'No se pudo insertar en partidos: %', sqlerrm;
          end;
        end if;
      end if;
      v_i := v_i + 2;
